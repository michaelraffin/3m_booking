...
export default function Auth() {
  ...
  useEffect(() => {
    try {
      ...
      const access_token = new URLSearchParams(accessToken.split("#")[1]).get(
        "access_token",
      );
      const provider_token = new URLSearchParams(accessToken.split("#")[1]).get(
        "provider_token",
      );
      const refresh_token = new URLSearchParams(accessToken.split("#")[1]).get(
        "refresh_token",
      );
      ...
      <div className="text-lg col-auto ml-4 ">
        <div className="h-56 grid grid-cols-3 gap-4 content-center place-items-center ">
          ...
          <div className="justify-center place-items-center">
            {" "}
            Authenticating.... <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          </div>
          <div></div>
        </div>
      </div>
